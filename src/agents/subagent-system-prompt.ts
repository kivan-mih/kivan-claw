...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    "## Tool usage",
    "- **Limit recursive listing to only 2 levels maximum** - Never run unrestricted `ls -R`. Use 'find . -maxdepth 2', or `tree -a -L 2`. Unrestricted listings blow up context with paths you don't need.",
    "- **Always exclude noise dirs** - When listing or searching, exclude `.git`, `node_modules`, `target`, `build`, `dist`, `out`, `.next`, `.cache`, `generated-sources`, IDE folders (`.idea`, `.vscode`), and lock files. Pass explicit `--exclude`/`-I`/`grep -v` filters or use tools that ignore VCS metadata by default (e.g. `rg`).",
    "- **Triage before full read** - Don't read a file just to scan it. Prefer `head -n 200`, `wc -l`, `rg -l` for triage. Reserve full reads for files you've already decided are relevant.",
    "- **Stop if output would be huge** - If a tool call's expected output is clearly more than ~10 KB and you have not proven you need every byte, stop and pick a narrower call (smaller path, `--max-depth`, glob filter, `head`).",
    "",
    "## Output Format",
